...
namespace mxnet {
namespace op {
DMLC_REGISTER_PARAMETER(NewReorgParam);

static bool NewReorgShape(const nnvm::NodeAttrs& attrs,
                       std::vector<TShape> *in_shape,
                       std::vector<TShape> *out_shape) {
    using namespace mshadow;
    const NewReorgParam& param_ = nnvm::get<NewReorgParam>(attrs.parsed);

    out_shape->resize(1, TShape());
    const TShape &dshp = (*in_shape)[newreorg::kData];
    ...
    SHAPE_ASSIGN_CHECK(*in_shape, newreorg::kData, dshape);
...
static bool NewReorgType(const nnvm::NodeAttrs& attrs,
...
struct NewReorgGrad {
    ...
    std::vector<nnvm::NodeEntry> operator()(const nnvm::NodePtr& n,
        ...
        heads.push_back(n->inputs[newreorg::kData]);
...
.set_num_outputs(1)
.set_attr_parser(NewReorgParamParser)
...
.set_attr<nnvm::FInferShape>("FInferShape", NewReorgShape)
.set_attr<nnvm::FInferType>("FInferType", NewReorgType)
.set_attr<FCompute>("FCompute<cpu>", NewReorgCompute<cpu>)
.set_attr<nnvm::FGradient>("FGradient", NewReorgGrad{"_backward_NewReorg"})
.add_argument("data", "NDArray-or-Symbol", "Input data to the ConvolutionOp.")
.add_arguments(NewReorgParam::__FIELDS__());
...
.set_attr_parser(NewReorgParamParser)
.set_attr<FCompute>("FCompute<cpu>", NewReorgGradCompute<cpu>);
