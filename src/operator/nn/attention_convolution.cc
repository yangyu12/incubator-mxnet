...
namespace mxnet {
namespace op {
DMLC_REGISTER_PARAMETER(AttentionConvolutionParam);

static inline index_t AddPad(index_t dsize, index_t pad) {
  return dsize + 2 * pad;
}

static inline std::vector<std::string> ListArguments(const AttentionConvolutionParam& param_) {
  if (!param_.no_bias) {
    return {"data", "attention", "weight", "bias"};
  } else {
    return {"data", "attention", "weight"};
  }
}

// shape infer method
static bool AttentionConvolutionShape(const nnvm::NodeAttrs& attrs,
                                      std::vector<TShape> *in_shape,
                                      std::vector<TShape> *out_shape) {
  using namespace mshadow;
  const AttentionConvolutionParam& param_ = nnvm::get<AttentionConvolutionParam>(attrs.parsed);
  if (!param_.no_bias) {
    CHECK_EQ(in_shape->size(), 4U) << "Input:[data, attention, weight, bias]";
  } else {
    CHECK_EQ(in_shape->size(), 3U) << "Input:[data, attention, weight]";
  }
  // CHECK_EQ(out_shape->size(), 1) << "Output: [output]";
  out_shape->resize(1, TShape());
  const TShape &dshp = (*in_shape)[attconv::kData];
  // const TShape &ashp = (*in_shape)[attconv::kAttention];
  if (dshp.ndim() ==  0) return false;

  if (param_.kernel.ndim() == 1) {
    // TODO: complete the shape inference for 1D case.
    // 1d conv
    ...
    // Shape<3> ashape = Shape3(dshape[0], param_.kernel[0] * dshape[1] / param_.num_group,
    //   dshape[2]);
    ...
    SHAPE_ASSIGN_CHECK(*in_shape, attconv::kWeight, wshape);
    if (!param_.no_bias) {
      SHAPE_ASSIGN_CHECK(*in_shape, attconv::kBias, Shape1(param_.num_filter));
    }
    ...
    SHAPE_ASSIGN_CHECK(*in_shape, attconv::kData,
    ...
    Shape<4> ashape = Shape4(dshape[0], param_.kernel[0] * param_.kernel[1] * dshape[1] / param_.num_group,
                             dshape[2], dshape[3]);
    Shape<4> wshape = Shape4(param_.num_filter / param_.num_group, dshape[1] / param_.num_group,
                             param_.kernel[0], param_.kernel[1]);
    ashape = ConvertLayout(ashape, kNCHW, param_.layout.value());
    wshape = ConvertLayout(wshape, kNCHW, param_.layout.value());
    wshape[0] *= param_.num_group;
    SHAPE_ASSIGN_CHECK(*in_shape, attconv::kAttention, ashape);  // Check whether attention shape is right.
    SHAPE_ASSIGN_CHECK(*in_shape, attconv::kWeight, wshape);
    if (!param_.no_bias) {
      SHAPE_ASSIGN_CHECK(*in_shape, attconv::kBias, Shape1(param_.num_filter));
    }
    ...
    // TODO: complete the shape inference for 3d case
    // 3d conv
...
// Type infer method: not changed.
static bool AttentionConvolutionType(const nnvm::NodeAttrs& attrs,
...
// storage type infer method
inline static bool AttConvStorageType(const nnvm::NodeAttrs& attrs,
                                   const int dev_mask,
...
struct AttentionConvolutionGrad {
  const char *op_name;
...
.set_num_inputs([](const NodeAttrs& attrs) {
  const AttentionConvolutionParam& params = nnvm::get<AttentionConvolutionParam>(attrs.parsed);
  return params.no_bias ? 3 : 4;
})
.set_num_outputs(1)
.set_attr_parser(AttentionConvolutionParamParser)
.set_attr<nnvm::FListInputNames>("FListInputNames",
    [](const NodeAttrs& attrs) {
  const AttentionConvolutionParam& params = nnvm::get<AttentionConvolutionParam>(attrs.parsed);
  if (params.no_bias)
    return std::vector<std::string>{"data", "attention", "weight"};
  else
    return std::vector<std::string>{"data", "attention", "weight", "bias"};
})
.set_attr<nnvm::FListOutputNames>("FListOutputNames",
    [](const NodeAttrs& attrs) {
    return std::vector<std::string>{"output"};
})
.set_attr<nnvm::FInferShape>("FInferShape", AttentionConvolutionShape)
.set_attr<nnvm::FInferType>("FInferType", AttentionConvolutionType)
.set_attr<FInferStorageType>("FInferStorageType", AttConvStorageType)
.set_attr<FCompute>("FCompute<cpu>", AttentionConvolutionCompute<cpu>)
.set_attr<nnvm::FGradient>("FGradient", AttentionConvolutionGrad{"_backward_AttentionConvolution"})
.set_attr<FResourceRequest>("FResourceRequest", [](const NodeAttrs& n) {
  return std::vector<ResourceRequest>{ResourceRequest::kTempSpace};
})
.add_argument("data", "NDArray-or-Symbol", "Input data to the ConvolutionOp.")
.add_argument("attention", "NDArray-or-Symbol", "Attention matrix.")
.add_argument("weight", "NDArray-or-Symbol", "Weight matrix.")
.add_argument("bias", "NDArray-or-Symbol", "Bias parameter.")
.add_arguments(AttentionConvolutionParam::__FIELDS__());

// register backward op: why we need to register a new backward op????
...
.set_attr<FInferStorageType>("FInferStorageType", BackwardAttConvStorageType)
.set_attr<FResourceRequest>("FResourceRequest", [](const NodeAttrs& n) {
  return std::vector<ResourceRequest>{ResourceRequest::kTempSpace};
})
.set_attr_parser(AttentionConvolutionParamParser)
.set_attr<FCompute>("FCompute<cpu>", AttentionConvolutionGradCompute<cpu>);
